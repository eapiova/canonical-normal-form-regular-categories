...
(*
Lemma raw_term_soundness {Σ : signature σ} (Γ : raw_context Σ) (t : raw_term Σ Γ) : 
  (exists gamma, t = raw_variable gamma) \/ (exists symb (e : symbol_class symb = class_term) args, t = @raw_symbol' σ Σ Γ class_term symb e args).
Proof.
  destruct t.
  unfold raw_term in *.
  eexists.
  right.
  unfold raw_expression.
  induction t.
  left.
  destruct t.
  le
...
Proof.
  ...
  - unfold Metavariable.instantiation.
    apply t_args.
  - destruct R_S; destruct R_S_introduces_S.
    assert (H := raw_rule_conclusion).
    apply boundary_of_judgement in H.
    destruct raw_rule_conclusion.
    destruct hypothetical_part.
    recursive_destruct form_of_judgement; simpl in *.    
    + destruct H.
      destruct hypothetical_part_of_boundary.
      unfold hypothetical_boundary_expressions in boundary_expression.
      destruct form_of_boundary; admit.
  Admitted.

  (*
      unfold Judgement.boundary_slot in boundary_expression.
      destruct boundary_expression.
      recursive_destruct form_of_boundary; admit.
    + destruct H.
    destruct hypothetical_part_of_boundary.
    recursive_destruct form_of_boundary.
    recursive_destruct boundary_expression.
    recursive_destruct form_of_boundary.
    + destruct symbol_rule_is_object0.
    + destruct symbol_rule_is_object0.
  
  destruct R_S_introduces_S. 
    rewrite <- symbol_rule_metas0.
    rewrite <- symbol_rule_context0.
    destruct R_S_tight.
    simple refine (raw_symbol' _ _ _).
    recursive_destruct (symbol_rule_class0).
